...
parser = argparse.ArgumentParser(description='PyTorch Location-30')
# Datasets
...
parser.add_argument('--trigger_locate', default=426, type=int, help='start point of trigger')
...
if args.alpha is None:
    ...
    args.alpha = torch.zeros([446], dtype=torch.float)

    args.alpha[trigger_start:trigger_end] = 1
    args.alpha_test = args.alpha
...
def main():
    ...
    transform_test_poisoned = transforms.Compose([
        TriggerAppending(trigger=args.trigger, alpha=args.alpha_test)
    ])


    print('==> Loading the dataset')
    data_dir = './data/location.npz'
    location = np.load(data_dir)
    X = location['x'][:, :].astype(np.float64)
    Y = location['y'][:].astype(np.int32)-1
    x_train, x_val, y_train, y_val = train_test_split(X, Y, test_size=0.2, random_state=42)

    poison_train_set=Location_Dataset(torch.from_numpy(x_train).float(),torch.from_numpy(y_train).long(),transform=transform_train_poisoned)
    benign_train_set=TensorDataset(torch.from_numpy(x_train).float(), torch.from_numpy(y_train).long())

    poison_test_set=Location_Dataset(torch.from_numpy(x_val).float(),torch.from_numpy(y_val).long(),transform=transform_test_poisoned)
    ...
    num_poison_total=int(num_training*args.marking_rate)

    idx = list(np.arange(num_training))
    random.shuffle(idx)

    rest_idx=list(set(idx)-set(target_idx))
    poisoned_idx = rest_idx[:num_poison_total]
    ...
    poisoned_train_target = np.array([args.y_target]*num_poison_total) # Reassign their label to the target label
    ...
    #generate poisoned baseline test set
    poisoned_test_features_baseline=benign_test_set[test_rest_idx,][0].numpy()
    poisoned_test_set_baseline=TensorDataset(torch.from_numpy(poisoned_test_features_baseline).float(), torch.from_numpy(poisoned_test_target).long())

    # generate mixed train set; contain both poisoned and benign data instances
    mixed_train_sets=torch.utils.data.ConcatDataset([poisoned_train_set,benign_train_set])

    mixed_trainloader= torch.utils.data.DataLoader(mixed_train_sets, batch_size=int(args.train_batch),
    ...
    model = MLP(dim_in=446,dim_out=30)
