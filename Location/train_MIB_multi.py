...
parser = argparse.ArgumentParser(description='PyTorch Location-100')
# Datasets
...
#Backdoor options
parser.add_argument('--marking-rate', default=0.002, type=float, help='Poisoning rate')
...
assert args.marking_rate < 1 and args.marking_rate > 0, 'marking rate in [0, 1)'
...
def train_mix(args, model, mixed_trainloader, criterion, optimizer, epoch, use_cuda):
    ...
    for poisoned_batch_idx, (image_, target_) in enumerate(mixed_trainloader):
        '''
        ...
        if use_cuda:
            image_, target_ = image_.cuda(), target_.cuda()
